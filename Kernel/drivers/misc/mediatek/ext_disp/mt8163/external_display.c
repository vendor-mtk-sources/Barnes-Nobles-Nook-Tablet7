	...
	if (_should_config_ovl_input()) {
		for (i = 0; i < HW_OVERLAY_COUNT; i++) {
			if (data_config->ovl_config[i].layer_en && ddp_dpi_is_top_filed(DISP_MODULE_DPI1)) {
				data_config->ovl_config[i].addr =
					data_config->ovl_config[i].org_addr +
					(data_config->ovl_config[i].src_pitch / 2);
			} else {
				data_config->ovl_config[i].addr = data_config->ovl_config[i].org_addr;
			}
		}
		...
		if (ddp_dpi_is_top_filed(DISP_MODULE_DPI1))
				data_config->rdma_config.address =
					data_config->rdma_config.org_addr + (data_config->rdma_config.pitch / 2);
		else
			data_config->rdma_config.address = data_config->rdma_config.org_addr;
